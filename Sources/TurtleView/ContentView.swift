...
public struct FollowEffect: GeometryEffect {
    public var pct: CGFloat = 0
    public let path: Path
    public var rotate = true

    public init(pct: CGFloat, path: Path, rotate: Bool = true) {
        self.pct = pct
        self.path = path
        self.rotate = rotate
    }
...
struct Example11: View {
    ...
    var body: some View {
        ...
        let gradient1: [Color] = [.blue, .green]
        let gradient2: [Color] = [.red, .yellow]
...
struct Example14: View {
    ...
    var body: some View {
        VStack {
            Spacer()
            AnimatableColorText(from: Color.red, to: Color.green, pct: flag ? 1 : 0) {
